...
const TodoList: React.FC<TodoListProps> = React.memo(({ isOpen, onClose, searchbarRef, isLoggedIn, onRequireLogin, isSidebar, isWidget, widgetSizePreset = 'small', showWidgetDisplayModeControl = false, isCreateModalOnly }) => {
 ...
 } else if (['command', 'automation', 'agent', 'chat_agent', 'aiprompt', 'ai_prompt', 'prompt'].includes(itemCat)) {
 chrome.tabs.create({
 url: chrome.runtime.getURL(
 ...
 if (cat === 'prompt' || cat === 'aiprompt' || cat === 'ai_prompt') return 'AI Prompt';
 ...
 if (catLower === 'prompt' || catLower === 'aiprompt' || catLower === 'ai_prompt') return 'AI Prompt';
 ...
 const configIds = task.config?.id;
 if (Array.isArray(configIds) && configIds.length > 1) {
 return wrapIcon(<SessionGridIcon size={iconSize - 2} className={wrap ? "text-[var(--color-textPrimary)]" : ""} />);
 }
 ...
 const taskCheckButtonClass = isLargeTodoWidget ? 'w-[20px] h-[20px]' : isMediumTodoWidget ? 'w-[18px] h-[18px]' : 'w-[16px] h-[16px]';
 ...
 >
 <FaCheck size={taskCheckIconSize} className="opacity-0 group-hover/check:opacity-100 text-emerald-500 transition-opacity" />
 </button>
 </div>
 )}
 {!isSidebar && !isWidget && (
 ...
 <div className={`flex items-center gap-1 mt-0.5 ${task.is_done ? 'text-[var(--color-textPrimary)]/30' : (theme.wallpaper ? 'text-[var(--color-textSecondary)]' : 'text-[var(--color-textMuted)]')}`}>
 <span className={`${taskMetaClass} font-medium tracking-wide whitespace-nowrap ${Array.isArray(task.config?.id) && task.config.id.length > 1
 ...
 {isSidebar ? (
 <div className={`grid ${deadlineDate.getTime() !== 0 ? 'grid-cols-[70px_10px_90px]' : 'grid-cols-[170px]'} items-center justify-end text-[11px] font-medium text-[var(--color-textMuted)]`}>
 ...
 <span className={`${task.is_done ? 'text-[var(--color-textMuted)]' : 'text-[var(--color-textMuted)]'} ${taskDateClass} font-medium flex items-center gap-1.5`}>
 <span>
 {isSameDay(deadlineDate, now)
 ? deadlineDate.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })
 ...
 <h4 className="text-[13px] font-semibold text-[var(--color-textPrimary)] tracking-wide mb-1">
 {title}
 </h4>
 ...
 const todoHeaderPaddingClass = isLargeTodoWidget ? 'py-1.5' : isWidget ? 'py-1' : isSidebar ? 'py-2.5' : 'py-4';
 const todoContentPaddingClass = isWidget ? 'px-4 pt-0' : 'px-4 pt-4';
 ...
 const todoAddButtonClass = isLargeTodoWidget ? 'w-9 h-9 mr-4' : isMediumTodoWidget ? 'w-8 h-8 mr-3' : `w-7 h-7 ${isWidget ? 'mr-3' : ''}`;
 ...
 <div className={`flex flex-col w-full max-w-5xl bg-transparent ${isCreateModalOnly ? 'hidden' : ''} ${isWidget ? 'h-full' : 'max-h-full'}`}>
 <div
 style={isSidebar || isWidget ? { maxHeight: '100%', height: '100%' } : { maxHeight: `${listHeight}px`, height: 'auto' }}
 ...
 <div className={`flex items-center justify-between px-2 shrink-0 w-full box-border border-b border-[var(--color-borderDefault)] group/header-tabs ${todoHeaderPaddingClass}`}>
 <div className="flex items-center gap-4">
 <button
 onClick={() => { setActiveSection('today'); setSelectedIndex(0); }}
 className={`${todoTopTabTextClass} font-medium tracking-wide transition-all flex items-center gap-1.5 ${activeSection === 'today' ? 'text-[var(--color-textPrimary)] font-bold' : 'text-[var(--color-textSecondary)] hover:text-[var(--color-textPrimary)]'}`}>
 <FiClock size={todoTopTabIconSize} />
 <span>Today</span>
 </button>
 <button
 onClick={() => { setActiveSection('scheduled'); setSelectedIndex(0); }}
 className={`${todoTopTabTextClass} font-medium tracking-wide transition-all flex items-center gap-1.5 ${activeSection === 'scheduled'
 ...
 <FiCalendar size={todoTopTabIconSize} />
 <span>Scheduled</span>
 </button>
 </div>
 ...
 {hoveredMode && gifPreviewPos && createPortal(
 <div
 style={{
 ...
 <span className="text-[var(--color-textPrimary)] font-semibold text-sm tracking-wide">All done</span>
 ) : (
 <span className="text-[var(--color-textPrimary)] font-medium text-sm tracking-wide">Pending ({activeTasks.length})</span>
 )}
 </div>
 ) : activeSection === 'today' ? (
 <>
 ...
 <div className={`flex items-center gap-1.5 font-bold ${todoSectionTextClass} tracking-[0.08em] text-[var(--color-textMuted)]`}>
 {collapsedGroups.active ? <FiChevronRight size={todoSectionIconSize} className="opacity-70" /> : <FiChevronDown size={todoSectionIconSize} className="opacity-70" />}
 ...
 <div className={todoSectionHeaderInnerClass}>
 <div className={`flex items-center gap-1.5 font-bold ${todoSectionTextClass} tracking-[0.08em] text-[var(--color-textMuted)]`}>
 {collapsedGroups.overdue ? <FiChevronRight size={todoSectionIconSize} className="opacity-70" /> : <FiChevronDown size={todoSectionIconSize} className="opacity-70" />}
 ...
 <div className={todoSectionHeaderInnerClass}>
 <div className={`flex items-center gap-1.5 font-bold ${todoSectionTextClass} tracking-[0.08em] text-[var(--color-textMuted)]`}>
 {collapsedGroups.completed ? <FiChevronRight size={todoSectionIconSize} className="opacity-70" /> : <FiChevronDown size={todoSectionIconSize} className="opacity-70" />}
 ...
 <div className={todoSectionHeaderInnerClass}>
 <div className={`flex items-center gap-2 font-bold ${todoSectionTextClass} tracking-[0.08em] text-[var(--color-textMuted)]`}>
